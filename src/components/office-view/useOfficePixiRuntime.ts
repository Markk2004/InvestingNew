...
import { buildSpriteMap } from "@/lib/spriteMap";
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      await app.init({
        ...
        height: 600,
        backgroundAlpha: 0,
        antialias: true, // Enable antialiasing for smoother text and vectors
        resolution: Math.max(window.devicePixelRatio || 1, 2), // Force at least 2x resolution for high-DPI clarity
        autoDensity: true,
      });
      ...
      // Removed pixelated imageRendering to allow smooth fonts and vectors
      element.innerHTML = "";

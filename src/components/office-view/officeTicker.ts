...
export function runOfficeTickerStep(ctx: OfficeTickerContext): void {
  ...
  for (const { sprite, status, baseX, baseY, particles, agentId, cliProvider, deskG, bedG, blanketG } of ctx
    ...
    if (status === "idle") {
      ...
      if (!sp._wanderTimer || sp._wanderTimer <= 0) {
        sp._wanderTimer = Math.random() * 200 + 100; // frames to walk or wait
        if (Math.random() > 0.5) {
          // Walk
          const angle = Math.random() * Math.PI * 2;
          sp._wanderDx = Math.cos(angle) * 0.5;
          sp._wanderDy = Math.sin(angle) * 0.5;
        } else {
          // Stand still
          sp._wanderDx = 0;
          sp._wanderDy = 0;
      ...
      const newX = sprite.position.x + (sp._wanderDx || 0);
      const newY = sprite.position.y + (sp._wanderDy || 0);
      
      // Keep within 100px of desk
      const distToDesk = Math.hypot(newX - baseX, newY - baseY);
      if (distToDesk < 80) {

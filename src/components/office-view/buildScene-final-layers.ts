...
import { Container, Graphics, Sprite, AnimatedSprite, Text, TextStyle, type Application, type Texture } from "pixi.js";
import type { Agent, Task } from "@/lib/agents";
import { CEO_SIZE, DESK_H, TARGET_CHAR_H, type Delivery } from "./model";
...
export function buildFinalLayers({
  ...
  const ceoAgent = agents.find(a => a.id === "mxrk");
  const spriteNum = ceoAgent ? (spriteMap.get(ceoAgent.id) ?? 1) : 1;
  ...
  if (frames.length > 0) {
    ...
    ceoCharacter.addChild(animSprite);
  } else {
    const fallback = new Graphics();
    fallback.circle(0, -18, 18).fill(0xff4d4d);
    ceoCharacter.addChild(fallback);
  }

  const crown = new Text({ text: "👑", style: new TextStyle({ fontSize: 14 }) });
  crown.anchor.set(0.5, 1);
  crown.position.set(0, -TARGET_CHAR_H - 2);
  ceoCharacter.addChild(crown);
  crownRef.current = crown;

  const nameBadge = new Graphics();
  nameBadge.roundRect(-16, -TARGET_CHAR_H - 20, 32, 11, 3).fill({ color: 0xf0d888, alpha: 0.9 });
  ...
  nameText.position.set(0, -TARGET_CHAR_H - 14.5);
